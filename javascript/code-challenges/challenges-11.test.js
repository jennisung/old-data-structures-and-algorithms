...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((count, arr) => count.concat(arr), []).filter(num => num === target).length;
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((totalSum, array) => totalSum + array.reduce((sum, num) => sum + num, 0), 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  arr.filter((numArr) => typeof numArr ==='number' && numArr % 5===0)
      .map((numArr) => Math.pow(2, numArr))
  );
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let namesOfCharac = data.filter((charac) => {
    return charac.gender ==='female' || charac.gender ==='male';
  }).map((charac) => {
    return charac.name;
  });
  return namesOfCharac.join(' and ');
};
...
let findShortest = (data) => {
  // Solution code here...
  let short  = data.reduce((characOne, characTwo) => {
    return Number(characOne.height) < Number(characTwo.height) ? characOne : characTwo;
  });
  return short.name;
};
