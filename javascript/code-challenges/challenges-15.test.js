...
const screenForNames = (arr) => {
  ...
  let newArray=[];
  for (let i of arr){
    if (/^(?:Mr\.|Mrs\.|Ms\.|Dr\.)\s[A-Za-z]+$/gm.test(i))
    {
      newArray.push(i);
    }
  }
...
const toTitleCase = (arr) => {
  // Solution code here...
  return arr.map(arrayOfStr => arrayOfStr.toUpperCase().charAt(0) + arrayOfStr.slice(1));
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let newStr = '';
  let moreThanLuke = arr.filter((weight) => weight.mass > 77);

  moreThanLuke.forEach(characters=> {
    newStr === '' ? newStr += characters.name : newStr += ' - ' + characters.name;
  });
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  let checkRowColumn = (row1, col1, row2, col2, row3, col3) => {
  ...
  if (checkRowColumn(0, 0, 1, 1, 2, 2)) {
    return true;
  }
  if (checkRowColumn(2, 0, 1, 1, 0, 2)) {
    return true;
  }
  for (let i = 0; i < 3; i++) {
    if (checkRowColumn(i, 0, i, 1, i, 2)) {
      return true;
    }
    if (checkRowColumn(0, i, 1, i, 2, i)) {
      return true;
    }
  }
  return false;
