...
const addOne = (arr) => {
  ...
  arr.forEach(element => {
    let addition = element + 1;
    incrementOne.push(addition);
  });
  return incrementOne;
};
...
const forLoopTwoToThe = (arr) => {
  ...
  for(let i = 0; i < arr.length; i++) {
    let secondPower = Math.pow(2, arr[i]);
    theArray.push(secondPower);
  }
  return theArray;
};
...
const forEachTwoToThe = (arr) => {
  ...
  arr.forEach((element) => {
    let secondPow = Math.pow(2, element);
    raisedSecond.push(secondPow);
  });
